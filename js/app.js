/* ============================================================
   LEXIPROF — APP.JS COMPLET v6.1
   QCM supprime | Quick actions supprimees | Loader rapide
   Border-left matiere | Copier/Partager | Recherche sticky
   ============================================================ */
...
/* ============================================================
   ...
   ============================================================ */
const defaultData = [
  { id: 1, term: "Management", matiere: "Management", def: "Ensemble des techniques permettant de diriger, organiser et coordonner les ressources d'une organisation afin d'atteindre ses objectifs.", example: "Un manager organise le travail de son equipe, repartit les taches et suit les resultats.", remember: "Le management consiste notamment a organiser, decider, coordonner et motiver." },
  { id: 2, term: "Leadership", matiere: "Management", def: "Capacite d'une personne a guider, influencer et motiver un groupe d'individus vers la realisation d'objectifs communs.", example: "Un responsable qui motive son equipe autour d'un projet fait preuve de leadership.", remember: "Leadership = influencer, guider et motiver." },
  { id: 3, term: "Organigramme", matiere: "Management", def: "Representation graphique de la structure hierarchique et fonctionnelle d'une organisation.", example: "L'organigramme d'une entreprise permet de voir qui depend de quel responsable.", remember: "Il permet de visualiser les relations hierarchiques." },
  { id: 4, term: "Contrat de travail", matiere: "Droit", def: "Convention par laquelle une personne, le salarie, s'engage a travailler pour le compte et sous la direction d'un employeur en echange d'une remuneration.", example: "Un CDI signe entre une entreprise et un salarie constitue un contrat de travail.", remember: "Travail + remuneration + lien de subordination." },
  { id: 5, term: "Personne morale", matiere: "Droit", def: "Entite juridique distincte des personnes physiques qui la composent, dotee de droits et d'obligations propres.", example: "Une societe ou une association peut etre une personne morale.", remember: "Une personne morale possede une existence juridique propre." },
  { id: 6, term: "PIB", matiere: "Economie", def: "Produit Interieur Brut. Indicateur macroeconomique mesurant la valeur totale des biens et services produits sur le territoire national au cours d'une periode donnee.", example: "Le PIB permet notamment de mesurer la production economique d'un pays.", remember: "PIB = valeur des biens et services produits sur un territoire." },
  { id: 7, term: "Inflation", matiere: "Economie", def: "Hausse generalisee et durable du niveau des prix des biens et services dans une economie.", example: "Lorsque les prix augmentent durablement, le pouvoir d'achat de la monnaie diminue.", remember: "Inflation = hausse generale et durable des prix." },
  { id: 8, term: "Recrutement", matiere: "RH", def: "Processus par lequel une organisation identifie, attire et selectionne des candidats afin de pourvoir un poste vacant.", example: "Une entreprise publie une offre d'emploi puis selectionne les candidats.", remember: "Recruter = rechercher, selectionner puis integrer un candidat." },
  { id: 9, term: "GPEC", matiere: "RH", def: "Gestion Previsionnelle des Emplois et des Competences. Demarche permettant d'anticiper les besoins futurs d'une organisation en emplois et en competences.", example: "Une entreprise peut prevoir des formations pour preparer ses salaries a de nouvelles competences.", remember: "GPEC = anticiper les besoins futurs en emplois et competences." }
];
...
function getPassword() { return localStorage.getItem(PWD_KEY) || DEFAULT_PW; }
  ...
  if (on) {
    container.innerHTML = `
      <div class="empty-state">
        <div class="icon">⏳</div>
        <h3>Chargement...</h3>
        <p>Recuperation des definitions.</p>
      </div>
    `;
  }
}
...
function render() {
  ...
  if (info) {
    if (searchQuery || currentFilter !== "all") {
      info.textContent = `${filtered.length} resultat${filtered.length !== 1 ? "s" : ""}`;
    } else {
      info.textContent = `${definitions.length} definition${definitions.length !== 1 ? "s" : ""}`;
    }
  }
  ...
  container.innerHTML = filtered.map((d, i) => createCardHTML(d, i)).join("");
}

/* ============================================================
   CARTE (border-left, copier, partager, favori)
   ============================================================ */
function createCardHTML(d, index) {
  const isFav = favorites.includes(Number(d.id));
  return `
    <article class="card" data-id="${d.id}" data-matiere="${escapeHTML(d.matiere)}" style="animation-delay:${Math.min(index * 30, 300)}ms">
      <div class="card-top">
      ...
      <div class="card-actions-row">
        ...
        <button class="card-action-btn fav-btn ${isFav ? "active" : ""}" onclick="toggleFavorite(${d.id})" title="Favori">${isFav ? "★" : "☆"}</button>
      </div>
      <div class="card-extra" id="card-${d.id}">
        ${d.example ? `<div class="extra-block"><h4>💡 Exemple concret</h4><p>${escapeHTML(d.example)}</p></div>` : ""}
        ${d.remember ? `<div class="extra-block"><h4>📝 A retenir</h4><p>${escapeHTML(d.remember)}</p></div>` : ""}
      </div>
    </article>
  `;
}
...
function updateCounter() {
  ...
  if (!counter) return;
  counter.textContent = `${definitions.length} definition${definitions.length !== 1 ? "s" : ""} disponible${definitions.length !== 1 ? "s" : ""}`;
}
...
function renderAdminList() {
  ...
  if (!sorted.length) {
    list.innerHTML = `<p style="color:var(--text-muted);font-size:13px;text-align:center;padding:16px 0;">Aucune definition.</p>`;
    return;
  }
...
function cancelEdit() {
  ...
  if (btnFormAction) btnFormAction.textContent = "+ Ajouter la definition";
  if (btnCancelEdit) btnCancelEdit.style.display = "none";
  if (formTitle) formTitle.textContent = "Ajouter une definition";
}
...
function editDefinition(id) {
  ...
  if (formTitle) formTitle.textContent = "Modifier la definition";
...
async function deleteDefinition(id) {
  ...
  try {
    ...
    render();
    renderAdminList();
    showToast("🗑 Definition supprimee.");
  } catch (error) {
    console.error(error);
    showToast("⚠️ Erreur de sauvegarde.");
  }
...
function changePassword() {
  ...
  if (!p1 || p1.length < 4) {
    showToast("⚠️ Minimum 4 caracteres.");
    return;
  }
  if (p1 !== p2) {
...
function importJson() {
  ...
  if (!input || !input.files || !input.files[0]) {
    showToast("⚠️ Selectionne un fichier JSON.");
    return;
  }
...
async function processImport(parsed) {
  ...
  if (!imported.length) throw new Error("Aucune definition trouvee.");
  ...
  render();
  renderAdminList();
  showToast(`✅ ${added} definition${added !== 1 ? "s" : ""} importee${added !== 1 ? "s" : ""}`);
}
...
function openFlashcards() {
  ...
  if (!flashcardList.length) {
    showToast("⚠️ Aucune definition disponible.");
    return;
  }
...
function renderFlashcard() {
  ...
  if (reveal) {
    ...
    reveal.textContent = "Cliquez pour reveler";
  }
...
function revealFlashcard() {
  ...
  reveal.textContent = hidden ? "Cliquez pour masquer" : "Cliquez pour reveler";
}
...
/* ============================================================
   DEFINITION ALEATOIRE
   ============================================================ */
function showRandomDefinition() {
  if (!definitions.length) {
    showToast("⚠️ Aucune definition.");
    return;
  }
...
async function installApp() {
  ...
  if (result.outcome === "accepted") {
    showToast("✅ LexiProf ajoute a l'ecran d'accueil !");
  }
